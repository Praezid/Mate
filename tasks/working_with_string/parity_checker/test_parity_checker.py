...
from tasks.working_with_string.parity_checker import parity_checker
...
"""
def test_when_number_is_odd():
    for number in [1, 3, 25, 111]:
        parity = 'Odd'
        prev_stdin = sys.stdin
        string_io_stdin = StringIO(str(number))
...
def test_when_number_is_even():
    for number in [0, 2, 24, 100]:
        parity = 'Even'
        prev_stdin = sys.stdin
        string_io_stdin = StringIO(str(number))
        sys.stdin = string_io_stdin
...
@pytest.mark.parametrize("number,parity", [
    ("0", "Even"),
    ("2", "Even"),
    ("24", "Even"),
    ("100", "Even"),
